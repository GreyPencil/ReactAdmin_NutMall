...
class ProductAddUpdate extends PureComponent {
  ...
  submit = () => {
    ...
    this.props.form.validateFields(async (error, values) => {
      if (!error) {
        ...
        // 3. 根据结果提示
        if (result.status===0) {
          message.success(`${this.isUpdate ? '更新' : 'Add'}Product成功!`)
          this.props.history.goBack()
        } else {
          message.error(`${this.isUpdate ? '更新' : 'Add'}Product失败!`)
        }
      }
    })
  }
  ...
  componentWillMount () {
    // 取出携带的state
    const product = this.props.location.state  // 如果是Add没值, 否则有值
    // 保存是否是更新的标识
  ...
  render() {
    ...
    const categoryIds = []
    if(isUpdate) {
      // Product是一个一级分类的Product
      if(pCategoryId==='0') {
        categoryIds.push(categoryId)
      } else {
        // Product是一个二级分类的Product
        ...
        <Form {...formItemLayout}>
          ...
          <Item label="Product Price">

            {
              ...
              })(<Input type='number' placeholder='Please input product price' addonBefore='$CAD'/>)
            }
          </Item>
