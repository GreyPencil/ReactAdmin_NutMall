...
## 4. ProductHome组件
    ...
    2). 搜索分页
       接口请求函数需要的数据: 
           ...
           productDesc / productName: searchName 根据Product描述/名称搜索
...
## 5. ProductDetail组件
    1). 读取Product数据: this.props.location.state.product
    2). 显示Product信息: <Card> / List 
    3). 异步显示Product所属分类的名称
...
## 1. ProductAddUpdate
    ...
    2). 分类的级联列表
        ...
        如果当前是更新二级分类的Product, 异步获取对应的二级分类列表, 生成二级分类options, 并Add为对应option的children
        async函数返回值是一个新promise对象, promise的结果和值由async函数的结果决定
        当选择某个一级分类项时, 异步获取对应的二级分类列表, 生成二级分类options, 并Add为当前option的children
...
## 2. PicturesWall
    ...
    4). 父组件调用子组件对象的方法: 使用ref技术
        ...
        2. 将ref容器交给需要获取的标签元素: <PicturesWall ref={this.pw} />  // 自动将将标签对象Add为pw对象的current属性
